...
class ContainerTest  {
    ...
    @Test
    fun `should unbind an existing binding and fail to resolve afterwards`() {
        val container = KoupperContainer()
        container.bind(AbstractClass::class, { ConcreteClass() })

        val instanceBefore = container.getInstance(AbstractClass::class)
        assertTrue(instanceBefore is ConcreteClass)

        container.unbind(AbstractClass::class)

        val exception = assertFailsWith<BindingException> {
            container.getInstance(AbstractClass::class)
        }

        assertTrue(exception.message!!.contains("not bound"))
    }

    @Test
    fun `should confirm if a type is bound in the container`() {
        val container = KoupperContainer()
        container.bind(AbstractClass::class, { ConcreteClass() })

        assertTrue(container.has(AbstractClass::class))
        container.unbind(AbstractClass::class)
        assertFalse(container.has(AbstractClass::class))
    }

    @Test
    fun `should reset all bindings and clear the container`() {
        val container = KoupperContainer()
        container.bind(AbstractClass::class, { ConcreteClass() })
        container.singleton(ParentAbstractClass::class, {
            ParentConcreteClass(FirstConcreteClass(ThirdConcreteClass()))
        })
    ...
    @Test
    fun `should resolve nested dependencies through constructor injection`() {
        val container = KoupperContainer("com.koupper.container.scope")

        val parent = container.getInstance(ParentAbstractClass::class)
        assertTrue(parent is ParentConcreteClass)

        val first = (parent as ParentConcreteClass).firstAbstractClass
        val third = (first as FirstConcreteClass).thirdAbstractClass

        assertTrue(first is FirstConcreteClass)
        assertTrue(third is ThirdConcreteClass)
    }

    @Test
    fun `should allow multiple singleton instances with different tags`() {
        val container = KoupperContainer()

        container.singleton(AbstractClass::class, { ConcreteClass() }, tag = "A")
        container.singleton(AbstractClass::class, { ConcreteClass2() }, tag = "B")

        val a = container.createSingleton(AbstractClass::class, tagName = "A")
        val b = container.createSingleton(AbstractClass::class, tagName = "B")

        assertTrue(a is ConcreteClass)
        assertTrue(b is ConcreteClass2)
