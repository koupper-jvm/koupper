...
class KoupperContainer() : Container {
    ...
    private fun KType.isNotAnyClass(): Boolean {
        return !(this.classifier as KClass<*>).simpleName.equals("Any")
    }

    private fun KClass<*>.isConcrete(): Boolean {
        return !this.isAbstract
    }

    private fun KType.asKClass(): KClass<*> {
    ...
    fun has(kClass: KClass<*>): Boolean {
        return this.bindings.containsKey(kClass) || this.singletons.containsKey(kClass)
    }

    fun unbind(kClass: KClass<*>) {
        this.bindings.remove(kClass)
        this.singletons.remove(kClass)
        this.bindingsMirror.remove(kClass)
        this.singletonMirror.remove(kClass)
        this.listeners.remove(kClass)
    }
}
