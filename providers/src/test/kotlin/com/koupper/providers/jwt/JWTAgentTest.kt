...
class JWTAgentTest : AnnotationSpec() {
    ...
    fun `should encode and decode a token`() {
        withEnvironment("JWT_SECRET", "7Xu2AiaG-iGX7FHii9aEy@MXuQkVEFdp") {
            ...
            assertEquals(decodeToken.claims["credentials"]!!, "876FSDFh7324")
            assertEquals(decodeToken.claims["expiresAt"]!!, 1636599153)
            assertEquals(decodeToken.claims["iv"]!!, "876FSDFh7233s")
        }
    }
