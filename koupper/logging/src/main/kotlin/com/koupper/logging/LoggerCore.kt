...
class PatternFormatter(
    private val pattern: String =
        "%d{yyyy-MM-dd HH:mm:ss.SSS} [%level] [%thread] %logger - %msg%ex%n"
) : LogFormatter {
...
class RollingFileAppender(
    ...
    override fun close() {
        // COMPROBACIÓN 2: ¿Quién y cuándo mató al Appender?
...
object Appenders {
    fun console(pattern: String = "%d{yyyy-MM-dd HH:mm:ss.SSS} [%level] [%thread] %logger - %msg%ex%n") =
    ...
    fun rollingFile(
        ...
        linePattern: String = "%d{yyyy-MM-dd HH:mm:ss.SSS} [%level] %logger - %msg%ex%n"
    ): Appender {
...
fun <T> captureLogs(
    ...
    val mem = MemoryAppender(
        PatternFormatter("%d{yyyy-MM-dd HH:mm:ss.SSS} [%level] %logger - %msg%ex%n")
    )
...
fun <T> captureLogsAsync(
    loggerName: String,
    spec: LogSpec,
    // El bloque ahora recibe el logger Y una función 'done' para cerrar el log
    block: (KLogger, () -> Unit) -> T
) {
    val logger = LoggerFactory.get(loggerName)
    val restore = configure(logger, spec)
    ...
    // Función de limpieza manual que antes estaba en el finally
    val closeLogs = {
        logger.removeAppender(mem)
        logger.removeFilter(captureFilter)
        mem.close()
    ...
    // Ejecutamos pasándole la referencia de cierre
    ScopedMDC(spec.mdc).use {
        block(logger, closeLogs)
    }
}
