...
data class KouTask(
    ...
    val signature: Pair<List<String>, String> = Pair(emptyList(), "Unit"),

    // --- Datos base ---
    val scriptPath: String? = null,            // ahora puede ser null (para pseudo-tasks)
    val packageName: String? = null,
    val origin: String = "manual",
    val context: String? = "default",
    val sourceType: String = "script",
...
object JobDispatcher {
    fun dispatch(task: KouTask, config: JobConfiguration, jobsDirectory: String? = "jobs") {
        val directory = ((jobsDirectory ?: "jobs")
            .trim('/', '\\')
            .takeIf { !it.contains('.') }
            ?: "jobs")
        ...
        when (config.driver) {
            "file" -> {
                ...
                val jobsDir = File("${rootPath}$directory/${config.queue}")
...
object JobRunner {
    ...
    fun runCompiled(context: String, task: KouTask): Any? {
        try {
            ...
            val value = when (val functionRef = field.get(null)) {
                is KProperty0<*> -> functionRef.get()
                else -> functionRef
            ...
            val args: List<Any?> = task.params.entries
                ...
                .mapIndexed { index, entry ->
                    val typeName = task.signature.first[index]
...
object JobBuilder {
    fun build(context: String): String {
        val command = if (System.getProperty("os.name").startsWith("Windows"))
            arrayOf("cmd", "/c", "gradlew.bat", "build")
        else
            arrayOf("./gradlew", "build")
...
private object FileJobFS {
    fun queueDir(queue: String, jobsDirectory: String? = "jobs"): File {
        val cleanDir = ((jobsDirectory ?: "jobs")
            .trim('/', '\\')
            .takeIf { !it.matches(Regex(".*\\.(json|yaml|yml|txt|kts)$", RegexOption.IGNORE_CASE)) } // evita archivos
            ?: "jobs")

        return File("$cleanDir/$queue")
    }
...
interface JobDriver {
    fun forEachPending(config: JobConfiguration, jobId: String?, jobsDirectory: String? = "jobs"): List<JobResult>
    fun listPending(context: String, config: JobConfiguration, jobId: String?, jobsDirectory: String? = "jobs"): List<JobResult>
}

interface ContextualJobDriver : JobDriver {
    fun forEachPending(context: String, config: JobConfiguration, jobId: String?, jobsDirectory: String? = "jobs"): List<JobResult>
}

object FileJobDriver : ContextualJobDriver {
    override fun forEachPending(
        context: String,
        ...
        val dir = File("$context${File.separator}$cleanDir${File.separator}${config.queue}")
    ...
    override fun listPending(
        ...
        return if (jobId.isNullOrBlank()) {
            ...
            results.filter {
                it is JobResult.Ok && it.task.id == jobId
            }.ifEmpty {
                listOf(JobResult.Error("⚠️ No job found with id '$jobId' in [${dir.absoluteFile}]"))
            }
        }
    }
}
...
fun KProperty0<*>.asJob(vararg args: Any?): KouTask {
    ...
    val finalScriptPathContent = File(finalScriptPath)
